...
void _start(void (*symreg)(char[24], uint64_t *), uint64_t *(*getsym)(char[24])) {
  mprintf("acpi: acpi module v0.1 by corruptmemory\n");
  uint64_t *(*get_rsdp)() = getsym("get_rsdp");
  mprintf("acpi: rsdp=0x%x\n", get_rsdp());
  struct XSDP_t *xsdt = (struct XSDP_t *)get_rsdp();
  mprintf("acpi: xsdt_address=0x%x, oemid=%s", xsdt->XsdtAddress, xsdt->OEMID);
}
