...
fn load_elf(data: &[u8], paging: &mut Paging) {
    ...
    // serial_println!("mod: {} LOAD sections", all_load_phdrs.len());
    ...
    for i in 0..all_load_phdrs.len() + 10 {
        ...
        unsafe { 
            //let page = Page::containing_address(VirtAddr::new(0xA000000 + (0xFFFFFF * MODULES_COUNT as u64) + (0x1000 * i as u64)));
    ...
    let code: extern "C" fn(fn([u8; 24], u64)) = unsafe { transmute(first_page.start_address().as_u64() + file.ehdr.e_entry - 0x400000) };
